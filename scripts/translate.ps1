...
function Invoke-SubtitleTranslator {
    ...
    if ($subtitleData.IsAutoGenerated) {
        if (-not $Quiet) {
            ...
            Write-Host "Using word-based segmentation..." -ForegroundColor Cyan
        }

        # Use word-based segmentation: AI inserts <br>, we split by word index
        $segments = Invoke-WordBasedSegmentation -Words $subtitleData.Words

        # Convert segments to entries with timestamps (direct index mapping)
