...
function Invoke-SubtitleTranslator {
    ...
    if ($subtitleData.IsAutoGenerated) {
        if (-not $Quiet) {
            ...
            Write-Host "Processing $($subtitleData.Cues.Count) cues..." -ForegroundColor Cyan
        }
        ...
        if (-not $Quiet) { Write-Host "Created $($entries.Count) subtitle entries" -ForegroundColor Green }
    } else {
